...
Remember to replace those values with your own in `src/Skoruba.IdentityServer8.Admin/appsettings.json` before first publish -
...
Last step before deploy - we need to update `src/Skoruba.IdentityServer8.STS.Identity/appsettings.json` and modify following lines:
